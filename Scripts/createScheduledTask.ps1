...
$BackupScriptPath = Join-Path -Path $PSScriptRoot -ChildPath "mongoBackup.ps1"

# Spezifiziert die auszuführende Aktion, in diesem Fall das Ausführen des Backup-Skripts
$ExecuteAction = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-NoProfile -WindowStyle Hidden -File `"$BackupScriptPath`""

# Registrieren der neuen geplanten Aufgabe im Task Scheduler
Register-ScheduledTask -TaskName "MongoDBDailyBackup" -Description "Tägliche Sicherung der MongoDB-Datenbank" -Trigger $ScheduledTime -Action $ExecuteAction -RunLevel Highest -User "NT AUTHORITY\SYSTEM"
